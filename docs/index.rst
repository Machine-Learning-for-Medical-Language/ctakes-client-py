.. ctakesclient documentation master file
...
``ctakesclient`` talks to a `cTAKES`_ natural language processing (NLP) server to annotate
fragments of text with appropriate medical `UMLS`_ Concepts and polarity.

For example, given the note ``Patient denies fever``, you can now detect:
...
.. _cTAKES: https://ctakes.apache.org/
.. _UMLS: https://nlm.nih.gov/research/umls
...
By default, ``ctakesclient`` assumes the cTAKES server is running locally at
``http://localhost:8080``.
...
See the `cnlp_transformers`_ project for more information on how that works.

.. _cnlp_transformers: https://github.com/Machine-Learning-for-Medical-Language/cnlp_transformers#negation-api
